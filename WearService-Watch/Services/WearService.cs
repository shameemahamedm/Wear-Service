namespace WearService_Watch.Services
{
    using System;
        ...
        public void SendMessage(string message)
        {
            Task.Run(() =>
            {
                foreach (var node in this.Nodes())
                {
                    ...
                    var result = WearableClass.MessageApi.SendMessage(this.client, node.Id, Path, bytes).Await();
                    var success = result.JavaCast<IMessageApiSendMessageResult>().Status.IsSuccess ? "Ok." : "Failed!";
                    Log.Info(Tag, "Communicator Sending message {0}... {1}", message, success);
                }
            });
        }
